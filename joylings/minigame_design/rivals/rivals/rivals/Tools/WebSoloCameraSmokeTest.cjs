...
(async()=>{const b=await chromium.launch({executablePath:process.env.RIVALS_CHROME,headless:true,args:['--enable-unsafe-swiftshader','--disable-background-timer-throttling','--disable-renderer-backgrounding','--disable-backgrounding-occluded-windows']});
 try{for(const mode of (process.argv.includes('--fallback')?['fallback']:['locked','fallback'])){
  ...
  await p.goto('http://localhost:8184/?diagnostics=1&v=solo-camera');await enterRoom(p,{name:'單人貓貓',room:'solo-camera'});
  ...
  const run={mode,initialRound:previous.round,maxIdleYaw:0,maxIdlePitch:0,maxCameraYaw:0,maxCameraPitch:0,maxHitRoll:0,botMovements:0,hits:0,death:false,nextRound:false,samples:[]};result.runs.push(run);
  const deadline=Date.now()+70000;
  // Deliberately keep hands off: bot locomotion, incoming shots, elimination
  // and round transitions occur naturally in the actual game.
  while(Date.now()<deadline){await p.waitForTimeout(110);const s=await state(),local=me(s),old=me(previous);
   const sample={phase:s.phase,round:s.round,health:local.health,damage:local.damage,fall:local.fall,look:s.look,camera:s.cameraAngles,position:s.cameraPosition};run.samples.push(sample);
   ...
   if(s.phase===2&&previous.phase===2&&s.round===previous.round&&local.health>0&&old.health>0){
   ...
   if(s.round>run.initialRound&&s.phase===1){run.nextRound=true;run.roundStart=sample;if(run.hits>0)break;}
   previous=s;
  }
  assert.ok(run.botMovements>20,'bots must move during observation');assert.ok(run.hits>0,'observe actual received damage');assert.ok(run.nextRound,'observe next round reset');assert.ok(run.maxHitRoll<=2.5,'alive hit reaction must remain a small roll');
